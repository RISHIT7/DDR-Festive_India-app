...
const Rating: React.FC<{ rating: number; count?: number; size?: number }> = ({ rating, count, size = 16 }) => (
    <div className="flex items-center">
        {[...Array(5)].map((_, i) => (
            <Star key={i} className={i < Math.round(rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'} size={size} />
        ))}
        ...
        <Link to={`/experiences/${experience.id}`}>
            ...
            <CardContent>
                <div className="flex justify-between items-center text-sm text-gray-600">
                    <span><Clock size={14} className="inline mr-1" />{experience.durationMins} mins</span>
                    <Rating rating={experience.rating} />
                </div>
            </CardContent>
            <CardFooter>
                <span className="text-xl font-bold text-[--color-indigo-dark]">₹{experience.priceINR}</span>
            </CardFooter>
        </Link>
    </Card>
);
    ...
    <div className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center">
                <h2 className="text-3xl font-bold font-serif text-gray-900">Why Festive India?</h2>
                <p className="mt-4 text-lg text-gray-600">Your gateway to authentic and unforgettable cultural journeys.</p>
            </div>
            <div className="mt-12 grid md:grid-cols-3 gap-10">
                <div className="text-center">
                    <div className="flex items-center justify-center h-12 w-12 rounded-full bg-orange-100 text-[--color-terracotta] mx-auto">
                        <Award size={28} />
                    </div>
                    <h3 className="mt-5 text-lg font-semibold font-serif">Authentic Experiences</h3>
                    <p className="mt-2 text-gray-600">Handpicked, verified hosts ensure you get a genuine taste of Indian culture, not a tourist trap.</p>
                </div>
                <div className="text-center">
                    <div className="flex items-center justify-center h-12 w-12 rounded-full bg-orange-100 text-[--color-terracotta] mx-auto">
                        <Shield size={28} />
                    </div>
                    <h3 className="mt-5 text-lg font-semibold font-serif">Safety & Trust</h3>
                    <p className="mt-2 text-gray-600">We prioritize your well-being with safety-certified experiences and clear guidance on the ground.</p>
                </div>
                <div className="text-center">
                    <div className="flex items-center justify-center h-12 w-12 rounded-full bg-orange-100 text-[--color-terracotta] mx-auto">
                        <Heart size={28} />
                    </div>
                    <h3 className="mt-5 text-lg font-semibold font-serif">Curated with Love</h3>
                    <p className="mt-2 text-gray-600">From historical narratives to culinary delights, every experience is chosen to create lasting memories.</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <Dialog isOpen={isPrefsModalOpen} onClose={() => setIsPrefsModalOpen(false)} title="What are you looking for?">
                ...
                <div className="flex flex-wrap gap-3 mb-8">
                    {PREFERENCE_OPTIONS.map(pref => (
                        <button key={pref} onClick={() => handlePrefToggle(pref)} className={`px-4 py-2 rounded-full text-sm font-medium border-2 transition-colors ${selectedPrefs.includes(pref) ? 'bg-[--color-terracotta] text-white border-[--color-terracotta]' : 'bg-white text-gray-700 border-gray-300 hover:border-[--color-terracotta]'}`}>
                            {pref}
                        </button>
                    ))}
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2">
                        ...
                        <p className="text-lg text-gray-700 mb-8">{festival.summary}</p>
                                ...
                                <CardContent>
                                    <ul className="space-y-4">
                                        {festival.schedule.slice(0, 3).map(item => <li key={item.id} className="flex items-start gap-3">
                                            <div className="bg-orange-100 text-[--color-terracotta] rounded-lg p-2 text-center text-sm">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="grid lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 text-gray-600">
                    ...
                    <div className="mt-10 pt-8 border-t grid md:grid-cols-2 gap-8">
                        <div>
                     ...
                     {host && (
                        <Card className="mt-10 pt-8 border-t bg-gray-50">
                ...
                <div className="lg:col-span-1">
                    <Card className="sticky top-24 shadow-2xl">
                        <CardHeader>
                            <CardDescription className="text-3xl font-bold text-[--color-indigo-dark] font-sans">₹{experience.priceINR}</CardDescription>
                        ...
                        <CardContent className="space-y-4">
                            <div>
                                ...
                                <select id="slot" value={selectedSlot} onChange={e => setSelectedSlot(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-[--color-terracotta] focus:border-[--color-terracotta] sm:text-sm rounded-lg">
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">Provide comma-separated URLs for your experience images.</p>
                            </div>
